...
function submitForm() {
  localStorage.setItem(FORM_SUBMITTED_AT_STORAGE_ID, new Date().getTime());
...
function dismissForm() {
  localStorage.setItem(FORM_DISMISSED_AT_STORAGE_ID, new Date().getTime());
  hideModal();
}
...
export function handleNewsletterDiscountPopup() {
  ...
  /** No run if Rewardful cookie present */
  ...
  if (dismissedAt && new Date().getTime() - dismissedAt < 24 * 60 * 60 * 1000) {
    return;
  }
  ...
  function getFeatureFlag() {
    return window.posthog?.getFeatureFlag(
  ...
  function callback() {
    const delay =
      getFeatureFlag() === "test-15sec-delay" ? 15000 : defaultDelay;
